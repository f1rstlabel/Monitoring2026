<template>
  <div class="space-y-6">
        ...
        <div v-else class="bg-[#151517] border border-[#26262A] rounded-xl overflow-hidden">
          ...
          <table class="w-full text-left text-xs text-gray-300">
            ...
            <tbody class="divide-y divide-[#26262A]">
              <tr
                v-for="(row, idx) in reportStore.filteredRows"
                :key="row.deviceId"
    ...
    <div v-if="!reportStore.isLoading">
      <RecurringIssuesTable :devices="reportStore.flapDevices" />
    </div>
    <template v-else>
      <SkeletonTable :rows="2" :cols="5" />
    </template>

    <!-- Active Incidents Report Section -->
    <div v-if="!reportStore.isLoading" class="bg-[#151517] border border-[#26262A] rounded-xl overflow-hidden mt-6">
      <div class="px-5 py-3.5 border-b border-[#26262A] flex items-center justify-between">
