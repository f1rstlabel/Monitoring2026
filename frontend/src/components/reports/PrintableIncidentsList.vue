<template>
  <Teleport to="body">
    <div id="printable-incidents-list" class="hidden print:block bg-white text-slate-900 p-0 print:p-0 font-sans leading-normal text-xs w-full mx-auto border-0 shadow-none">
      <!-- Kop Surat Header -->
      <div class="border-b-2 border-slate-900 pb-4 mb-6 flex justify-between items-start">
        <div>
          ...
          <p class="text-[10px] font-mono text-slate-600 mt-0.5">Daftar Antrean Kejadian &amp; Outage Infrastruktur SANOC</p>
        </div>
      ...
      <div class="mb-6">
        ...
        <template v-if="groupingMode !== 'none'">
          <div v-for="group in groupedIncidents" :key="group.name" class="mb-6">
            ...
            <table class="w-full text-left border-collapse border border-slate-300 text-xs mb-3">
              <colgroup>
                <col style="width: 15%;" />
                <col style="width: 25%;" />
                <col style="width: 15%;" />
              ...
              <tbody class="divide-y divide-slate-200">
                <tr v-for="inc in group.items" :key="inc.id" class="text-[11px]">
        ...
        <template v-else>
          <table class="w-full text-left border-collapse border border-slate-300 text-xs">
            ...
            <tbody class="divide-y divide-slate-200">
              <tr v-for="inc in incidents" :key="inc.id" class="text-[11px]">
                <td class="border border-slate-300 p-2 font-mono font-bold text-indigo-900">{{ inc.id }}</td>
                <td class="border border-slate-300 p-2 font-bold text-slate-800">{{ inc.deviceName }}</td>
                <td class="border border-slate-300 p-2 font-mono text-slate-600">{{ inc.deviceType }}</td>
                <td class="border border-slate-300 p-2 font-mono text-slate-600">{{ inc.deviceIp }}</td>
