  ...
  <div class="bg-[#151517] border border-[#26262A] rounded-xl p-5 space-y-4">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <h3 class="text-xs font-bold uppercase tracking-wider text-gray-300 font-mono">Flapping Frequency</h3>
      </div>
      <span class="text-[10px] font-mono text-gray-500 bg-[#18181B] px-2 py-0.5 rounded border border-[#26262A]">7-Day Window</span>
    </div>
    ...
    <div v-if="devices.length === 0" class="flex flex-col items-center justify-center h-44 text-center space-y-2">
      ...
      <p class="text-xs font-semibold text-gray-300">Zero Flapping Devices</p>
      <p class="text-[10px] text-gray-500 max-w-[200px]">All network devices are operating stably under threshold.</p>
    </div>
    ...
    <div v-else class="space-y-2">
      ...
      >
        ...
        <div class="w-36 shrink-0 text-right">
          <p class="text-[11px] text-gray-300 font-medium truncate group-hover:text-amber-400 transition-colors" :title="item.deviceName">
            {{ truncate(item.deviceName, 18) }}
          </p>
          <p class="text-[9px] font-mono text-gray-500 truncate">{{ item.deviceType }}</p>
        </div>
        ...
        <div class="flex-1 flex items-center gap-2">
          ...
          <span
            ...
            :class="item.downCount >= 10 ? 'text-[#F16565]' : 'text-amber-400'"
          >
            {{ item.downCount }}×
          </span>
        </div>
    ...
    <div v-if="devices.length > 0" class="grid grid-cols-2 gap-2 pt-2 border-t border-[#26262A]">
      <div class="bg-[#18181B] p-2.5 rounded-lg border border-[#26262A]">
        <p class="text-[9px] font-mono text-gray-400 uppercase">Impacted Nodes</p>
        <p class="text-base font-extrabold text-amber-400 font-mono mt-0.5">{{ devices.length }}</p>
      </div>
      <div class="bg-[#18181B] p-2.5 rounded-lg border border-[#26262A]">
        <p class="text-[9px] font-mono text-gray-400 uppercase">Total Flaps (7d)</p>
        <p class="text-base font-extrabold text-[#F16565] font-mono mt-0.5">{{ totalFlaps }}</p>
      </div>
    </div>

    <!-- Legend -->
    <div class="flex items-center gap-4 pt-1 text-[10px] font-mono text-gray-500">
