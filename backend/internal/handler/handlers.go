...
func (h *Handler) Login(c *gin.Context) {
	...
	validPassword := false
	if inputPass == "admin" || inputPass == "admin123" || inputPass == inputUser { // Fallback dev passwords
		validPassword = true
	} else {
		// Use a real hash check instead of placeholder if user has real hash.
		// Placeholder check for seeded data:
		if err := bcrypt.CompareHashAndPassword([]byte("$2a$10$8R3ySnqo4Ry9xsCCbu1CZu69ccq4t8UdrYMQcextliQh57qCo6bhC"), []byte(inputPass)); err == nil {
			validPassword = true
		}
	}
...
func (h *Handler) GetUserLogs(c *gin.Context) {
	...
	logs, err := h.userLogRepo.GetRecent(100)
	if err != nil {
...
func (h *Handler) GetMe(c *gin.Context) {
	...
	user, err := h.userRepo.GetByID(idStr)
	if err != nil || user == nil {
	...
	if h.permRepo != nil {
		perms, err := h.permRepo.GetByRole(user.Role)
		if err == nil {
			for _, p := range perms {
...
func (h *Handler) GetIncidents(c *gin.Context) {
	...
	c.JSON(http.StatusOK, []domain.Incident{})
}
...
func (h *Handler) GetIncidentByID(c *gin.Context) {
	...
	if h.incidentRepo != nil {
		...
		if err == nil && inc != nil {
			...
			// Ensure real timeline items exist
