...
func (h *IntegrationsHandler) CreateWhatsAppTarget(c *gin.Context) {
	...
	var jid string
	rawInput := req.PhoneNumber

	if req.TargetType == "group" || strings.HasSuffix(rawInput, "@g.us") {
		if strings.HasSuffix(rawInput, "@g.us") {
			jid = rawInput
		} else {
			gid := strings.ReplaceAll(rawInput, " ", "")
			jid = gid + "@g.us"
		}
	} else if strings.HasSuffix(rawInput, "@s.whatsapp.net") {
		jid = rawInput
	} else {
		// Normalize individual phone number to JID
		phoneNumberDigits := strings.ReplaceAll(rawInput, " ", "")
