...
func TestNotificationPipelineFailover(t *testing.T) {
	...
	pipeline := notifier.NewPipeline("http://invalid-whatsapp-host:9999", "MOCK_WA_TOKEN", "MOCK_TG_TOKEN", "-10012345", "+6281234567890", 60, nil, nil, nil)

	// Pipeline should attempt WhatsApp, encounter error, and failover gracefully
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()

	pipeline.Send(ctx, "🔴 TEST INCIDENT ALERT", nil)
...
// ─── Test 10: Concurrent Massive Outage Trigger Safety Test ───────────────────
func TestConcurrentOutageSpam(t *testing.T) {
	numDevices := 85
	devices := make([]domain.Device, numDevices)
	for i := 0; i < numDevices; i++ {
		devices[i] = domain.Device{
			ID:               fmt.Sprintf("dev-test-%d", i),
			Name:             fmt.Sprintf("Device Test %d", i),
			Type:             domain.AccessPoint,
			IP:               "192.0.2.1", // Reserved test IP (guarantees ping timeouts/failure)
			Status:           domain.StatusUP,
			FailureThreshold: 1, // trigger DOWN transition immediately on 1st fail
		}
	}

	deviceRepo := repository.NewMemoryDeviceRepository(devices)
	statusRepo := repository.NewMemoryStatusLogRepository()
	hub := ws.NewHub()
	go hub.Run()

	cfg := poller.DefaultConfig()
	cfg.ConcurrencyBatchSize = 100 // Process all concurrently
	cfg.Thresholds[domain.AccessPoint] = 1

	engine := poller.NewEngine(hub, deviceRepo, statusRepo, cfg)

	// Trigger polling cycle
	engine.TriggerPollNow()

	// Wait up to 6 seconds for the concurrent cycle workers to finish checking all 85 devices
	time.Sleep(6 * time.Second)

	// Assert that each device has transitioned exactly ONCE in the status logs
	logs := statusRepo.GetLogs()

	// Group logs by device ID
	countMap := make(map[string]int)
	for _, logEntry := range logs {
		if logEntry.Status == domain.StatusDOWN {
			countMap[logEntry.DeviceID]++
		}
	}

	// Verify that each device transitioned to DOWN exactly once
	for i := 0; i < numDevices; i++ {
		id := fmt.Sprintf("dev-test-%d", i)
		count := countMap[id]
		if count != 1 {
			t.Errorf("Device %s transitioned to DOWN %d times (expected exactly 1)", id, count)
		}
	}
}
