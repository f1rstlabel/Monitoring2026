...
app.post('/send', async (req, res) => {
  ...
  // Normalize number: strip non-digits, convert leading 0 to country code 62 (Indonesia)
  let formattedNum = String(num).replace(/[^0-9]/g, '');
  if (formattedNum.startsWith('0')) {
    formattedNum = '62' + formattedNum.slice(1);
  }
  const jid = formattedNum.includes('@s.whatsapp.net')
    ? formattedNum
    : `${formattedNum}@s.whatsapp.net`;

  // Bounded retry logic for transient socket reconnects (up to 10 attempts with 1s delay)
  let lastErr = null;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    if (connectionStatus !== 'connected' || !sock) {
      ...
      if (attempt < maxAttempts) {
        console.log(`[Baileys Sidecar] Session status '${connectionStatus}' (attempt ${attempt}/${maxAttempts}). Waiting 1s for socket ready...`);
    ...
    try {
      ...
      console.log(`[Baileys Sidecar] ✓ Message delivered to ${formattedNum} (attempt ${attempt})`);
      return res.json({ success: true, recipient: formattedNum });
    } catch (err) {
      lastErr = err;
      console.warn(`[Baileys Sidecar] Attempt ${attempt}/${maxAttempts} failed to send to ${formattedNum}: ${err.message}`);
      if (attempt < maxAttempts) {
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    }
  ...
  console.error(`[Baileys Sidecar] ✗ All ${maxAttempts} send attempts failed for ${formattedNum}:`, lastErr?.message);
  res.status(500).json({ error: lastErr?.message || 'Failed to send message after retries' });
});
